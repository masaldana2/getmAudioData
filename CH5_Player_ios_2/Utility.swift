...
class Utility {
   ...
   //
   static func check(_ error: OSStatus , operation: String) {
...
//let magicCookie: UnsafeMutableRawPointer  = malloc(4)
           let magicCookie = UnsafeMutablePointer<CChar>.allocate(capacity: Int(propertySize))

           // get the cookie
           Utility.check(AudioFileGetProperty (file,
           ...
           Utility.check(AudioQueueSetProperty(queue,
